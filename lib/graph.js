 ...
 * @param  {Object} config
 */
function setNodeColor(node, config, id, wrapEnv) {
	...
	const color = assetsColor || componentsColor || hocsColor || hooksColor || pagesColor || rootColor || utilsColor || '#ffffff';

	node.set('fillcolor', color);
	node.set('style', 'filled,rounded');
	if (wrapEnv) {
		node.set('color', config.utilsColor);
		node.set('penwidth', 3);
	}
}
...
function createGraph(modules, circular, config, options) {
	...
	Object.keys(modules).forEach((id) => {
		nodes[id] = nodes[id] || g.addNode(id.replace(/^(assets|hocs|pages|root|utils|hooks|components)\//, ''));

		const subModules = modules[id].filter(m => m.indexOf('utils/') < 0)
		const wrapEnv = modules[id].length !== subModules.length;
		// TODO: Set edge color for cyclics.
		setNodeColor(nodes[id], config, id, wrapEnv)


		subModules.forEach((depId) => {
			nodes[depId] = nodes[depId] || g.addNode(depId.replace(/^(assets|hocs|pages|root|utils|hooks|components)\//, ''));
